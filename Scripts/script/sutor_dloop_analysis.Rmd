...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/njmsaplot.svg", width = 11, height = 9)#save as svg file
njmsaplot
dev.off()
```
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/heatmap.svg", width = 7, height = 7)
heatmap(comat,scale="none",col=heat.colors(100),keep.dendro=TRUE, symm=TRUE)
dev.off()
```
...
dev.new()
plot(net, size=attr(net, "freq"), scale.ratio = 2, cex = 0.6, labels=TRUE, pie = ind.hap, show.mutation=2, font=2, fast=TRUE)
legend(x= 65,y=30, colnames(ind.hap), fill=rainbow(ncol(ind.hap)), cex=0.5, ncol=6, x.intersp=0,003, text.width=9)
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/hapind.svg", width = 11, height = 5)
par(mar=c(0.01,0.01,0.01,15))
plot(net, size=attr(net, "freq"), scale.ratio = 2, cex = 0.6, labels=TRUE, pie = ind.hap, show.mutation=1, font=2, fast=TRUE)
...
hname<-paste("H", 1:nrow(h), sep = "")
rownames(h)= paste(hname)
net<-haploNet(h, d = "dh", getProb = TRUE) #constructs the haplotype network
net
ind.hap<-with(
    utils::stack(setNames(attr(h, "index"), rownames(h))),
...
plot(net, size=attr(net, "freq"),shape= "circles", bg = bg, scale.ratio = 2, cex = 0.7, labels=F, pie = ind.hap, show.mutation=1, font=2, fast=TRUE)
legend(x=60,y=28, sn2, fill=bgp, cex=0.8, ncol=1, bty="n", x.intersp = 0.2)
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/happop.svg", width = 7, height = 4)
par(mar=c(0.001,0.001,0.001,0.001))
plot(net, size=attr(net, "freq"),shape= "circles", bg = bg, scale.ratio = 2, cex = 0.7, labels=F, pie = ind.hap, show.mutation=1, font=2, fast=TRUE)
legend(x=60,y=28, sn2, fill=bgp, cex=0.8, ncol=1, bty="n", x.intersp = 0.2)
...
nmname<-c(rep("Nature", 6), rep("Nature", 6), rep("Nature", 4), rep("Firm", 4), 
...
####Results
Caption:Haplotype networks for populations
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/njtreepop.svg", width = 9, height = 9.5)
emos<-ggtree(tree, layout = 'circular', branch.length='branch.length', lwd = 0.5)+xlim(-0.1, NA)
groupOTU(emos,krp, 'Populations') + aes(color=Populations)+theme(legend.position="right")+geom_tiplab(names(nbin), align=TRUE, linetype='dashed',linesize=.3, size=2.5, offset=0.002)+guides(color = guide_legend(override.aes = list(size = 2.5)))+geom_treescale(x=-0.1, color = "coral4", fontsize = 3, offset = 9)
```
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/njdistree.svg", width = 6.5, height = 6)
njdistree
```
...
```{r}
svg("~/Desktop/S-sutor/R_analysis/sutor_population_genetics/figures/boothap.svg", width = 11, height = 6)
boothap
